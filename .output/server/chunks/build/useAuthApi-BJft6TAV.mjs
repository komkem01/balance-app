...
const useAuthApi = () => {
  ...
  const getMyProfileImage = async () => {
    return await requestWithAuth("/me/profile-image", {
      method: "GET"
    });
  };
  ...
  const uploadMyTransactionSlip = async (walletID, imageFile) => {
    const formData = new FormData();
    formData.append("wallet_id", walletID);
    formData.append("image", imageFile);
    return await requestWithAuth("/balances/storage/slips", {
      method: "POST",
      body: formData
    });
  };
  const getMyTransactionSlip = async (transactionID) => {
    return await requestWithAuth(`/balances/storage/slips/${transactionID}`, {
      method: "GET"
    });
  };
  const getMyTransaction = async (transactionID) => {
...
//# sourceMappingURL=useAuthApi-BJft6TAV.mjs.map
